...
class TestValidateConfiguration:
    ...
    def test_validate_configuration_missing_dependencies(self, temp_dir):
        ...
        with patch('odysseus.core.validation.config_validators.check_dependencies') as mock_check:
    ...
    @patch('odysseus.core.validation.config_validators.check_dependencies')
    def test_validate_configuration_invalid_musicbrainz_delay(self, mock_check, temp_dir, monkeypatch):
        """Test validate_configuration with invalid MusicBrainz delay."""
        mock_check.return_value = (True, [])

        # Use monkeypatch to modify the config
        from odysseus.core.validation import config_validators
        original_config = config_validators.MUSICBRAINZ_CONFIG.copy()
        monkeypatch.setattr(config_validators, 'MUSICBRAINZ_CONFIG', {"REQUEST_DELAY": -1, "MAX_RESULTS": 3, "TIMEOUT": 30})
        ...
        try:
            ...
            monkeypatch.setattr(config_validators, 'MUSICBRAINZ_CONFIG', original_config)

    @patch('odysseus.core.validation.config_validators.check_dependencies')
    def test_validate_configuration_invalid_download_quality(self, mock_check, temp_dir, monkeypatch):
        """Test validate_configuration with invalid download quality."""
        mock_check.return_value = (True, [])

        # Use monkeypatch to modify the config
        from odysseus.core.validation import config_validators
        original_config = config_validators.DOWNLOAD_CONFIG.copy()
        monkeypatch.setattr(config_validators, 'DOWNLOAD_CONFIG', {"DEFAULT_QUALITY": "invalid", "AUDIO_FORMAT": "mp3", "MAX_CONCURRENT_DOWNLOADS": 3, "TIMEOUT": 300})
        ...
        try:
            ...
            # Restore original config
            monkeypatch.setattr(config_validators, 'DOWNLOAD_CONFIG', original_config)
...
class TestValidateAndRaise:
    ...
    def test_validate_and_raise_success(self, temp_dir):
        ...
        with patch('odysseus.core.validation.config_validators.validate_configuration') as mock_validate:
    ...
    def test_validate_and_raise_failure(self, temp_dir):
        ...
        # Mock validate_config to raise ConfigurationError
        with patch('odysseus.core.validation.config_validators.validate_config') as mock_validate:
            from odysseus.core.exceptions import ConfigurationError
            mock_validate.side_effect = ConfigurationError("Test error", details={"errors": ["Test error"]})
