...
class VideoValidator:
    ...
    VINYL_OR_LISTENING_PATTERNS = (
        r"\bvinyl\b",
        r"\bneedle\s*drop\b",
        r"\bturntable\b",
        r"\brecord\s+player\b",
        r"\blp\s+rip\b",
        r"\bvinyl\s+rip\b",
        r"\blistening\s+(?:session|party|experience)\b",
        r"\bfirst\s+time\s+listening\b",
    )
    ...
    def is_vinyl_or_listening_upload(
        self,
        video_title: str,
        video_info: Optional[dict] = None,
    ) -> bool:
        """Detect vinyl playback/rips and hosted listening-session uploads."""
        text_parts = [video_title or ""]
        if video_info:
            text_parts.extend([
                str(video_info.get("title") or ""),
                str(video_info.get("description") or ""),
            ])
        combined = " ".join(text_parts).lower()
        return any(
            re.search(pattern, combined)
            for pattern in self.VINYL_OR_LISTENING_PATTERNS
        )
    ...
    def validate_video_for_album(
        ...
        if self.is_vinyl_or_listening_upload(video.title):
            reason = f"Video appears to be a vinyl/listening-session upload (title: {video.title})"
            self._log_rejection(
                f"Skipping vinyl/listening upload: {video.title}",
                video,
                console,
                silent,
            )
        ...
        if self.is_vinyl_or_listening_upload(video.title, video_info):
            reason = f"Video metadata indicates vinyl playback or a listening session: {video.title}"
            self._log_rejection(
                f"Skipping vinyl/listening upload: {video.title}",
                video,
                console,
    ...
    def validate_video_for_track(
            ...
            if video_duration and track.duration:
                ...
                if expected_duration:
                    ...
                    if diff_ratio > threshold:
                        ...
                        if not silent and console:
                            console.print(
                                f"[yellow]⚠[/yellow] Skipping video: {reason}"
                            )
                            console.print(f"  [dim]YouTube: {video.youtube_url}[/dim]")
                        return False, reason
        ...
        if not duration_matches:
            ...
            if is_live:
                console.print(
                    "[blue]ℹ[/blue] Duration matches well; accepting despite "
                    "the title suggesting a live version"
                )
