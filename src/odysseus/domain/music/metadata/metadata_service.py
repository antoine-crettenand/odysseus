...
class MetadataService:
    ...
    def __init__(self, merger=None, cover_art_fetcher=None):
        """
        Initialize metadata service with dependencies.

        Args:
            merger: Optional MetadataMerger instance
            cover_art_fetcher: Optional CoverArtFetcher instance
        """
        if cover_art_fetcher is None:
            raise ValueError("MetadataService requires a cover-art fetcher")
        self.merger = merger or MetadataMerger()
        self.cover_art_fetcher = cover_art_fetcher
    ...
    def prepare_track_metadata_list(
        self,
        ...
        Args:
            track_timestamps: List of timestamp dicts with 'track' key
            release_info: Release information

        Returns:
            List of metadata dicts for each track
        """
    ...
    def apply_metadata_with_cover_art(
        ...
        try:
            ...
            date_to_use = (
                release_info.original_release_date or release_info.release_date
            )
            ...
            if success:
                ...
                if console:
                    console.print(
                        f"[yellow]⚠[/yellow] Failed to apply metadata to {track.title}; "
                        f"keeping downloaded file"
                    )
            ...
            # Keep the audio file — a tag write failure must not destroy a download
